...
import com.myp.user.service.UserService;
...
public class UserController {
	...
	public String loginProcess(HttpSession session,UserVO dto){
		...
		if ( vo != null ){ // 로그인 성공
			...
			returnURL = "redirect:/listAll"; // 로그인 성공시 게시글 목록페이지로 바로 이동하도록 하고
		}else { // 로그인에 실패한 경우
	...
	public String registerProcess(HttpSession session,UserVO dto){
		if ( session.getAttribute("login") != null ){
			// 기존에 login이란 세션 값이 존재한다면
